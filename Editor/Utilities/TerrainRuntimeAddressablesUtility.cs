...
public static class TerrainRuntimeAddressablesUtility
{
    ...
    // =====================================================
    // LEGACY TERRAIN STREAMING PREPARE
    // =====================================================

    /*
     * Preserve the existing explicit manual preparation behavior for height
     * and surface streams. Manual Prepare remains a structural reconciliation
     * command; the Package 07 planned path below is the optimized path.
     */
    public static bool PrepareTerrainStreamingAssets()
    {
        ...
        )
        {
            ...
            {
                Debug.LogWarning(
                    "Runtime height Addressables preparation was cancelled."
                );
            }
            else
            {
        ...
        )
        {
            ...
            {
                Debug.LogWarning(
                    "Runtime surface Addressables preparation was cancelled."
                );
            }
            else
            {
